...
def searchPage(Request):
    if (Request.method=="POST"):
        ...
        data=Employee.objects.filter(Q(name__contains=search)|Q(email__contains=search)|Q(phone__contains=search)|Q(dsg__contains=search)|Q(salary__contains=search)|Q(city__contains=search)|Q(state__contains=search))
        return render(Request,"index.html",{'data':data})
    else:
        return HttpResponseRedirect("/")
